	...
	function onAfterDispatch()
	{
		...
		if ($docType !== 'feed' && !JRequest::getInt('print') && !JRequest::getInt('noscript')) {
			...
			$script = '
				...
				var tooltips_sticky = '.(int) $this->params->sticky.';
			';
			$document->addScriptDeclaration(preg_replace('#\n\s*#s', ' ', trim($script)));
			$options = array();
			$options['maxTitleChars'] = 99999;
			$options['showDelay'] = (int) $this->params->show_delay;
			$options['fixed'] = (int) $this->params->fixed;
			$options['offsets'] = array('x' => (int) $this->params->offset_x,
										'y' => (int) $this->params->offset_y);
		...
		if (empty($buffer) || is_array($buffer) || strpos($buffer, '{'.$this->params->tag) === false) {
			return;
		}
